...
export default function ManageTypes() {
  const { types, typeColors, titles, addType, updateType, deleteType } =
    useData();
  const [editing, setEditing] = useState(null); // { name, color, isNew }
  const [deleting, setDeleting] = useState(null);

  const startAdd = () =>
    setEditing({ name: "", color: "#0D7377", isNew: true, originalName: "" });
  const startEdit = (name) =>
    setEditing({
      name,
      color: typeColors[name] || "#0D7377",
      isNew: false,
      originalName: name,
    });
  ...
  const save = () => {
    ...
    if (editing.isNew) {
      addType(editing.name.trim(), editing.color);
    } else {
      updateType(editing.originalName, editing.name.trim(), editing.color);
    }
    setEditing(null);
  };
    ...
    <div>
          ...
          >
            <div className="flex items-center gap-3">
              <div
                ...
                style={{ backgroundColor: typeColors[name] || "#888" }}
              >
                <TypeIcon type={name} className="size-4" />
              </div>
              <div>
      ...
      {deleting && (
        <ConfirmDialog
          title="Delete Type"
          message={
            titlesOfType(deleting) > 0
              ? `"${deleting}" has ${titlesOfType(deleting)} title(s). They will lose their type. Continue?`
              : `Delete "${deleting}"? This cannot be undone.`
          }
